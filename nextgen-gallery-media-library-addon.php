 ...
 */
/*
This file is part of NextCellent Gallery Media Library Addon.
NextCellent Gallery Media Library Addon is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.
NextCellent Gallery Media Library Addon is distributed in the hope that it will be useful,
...
define( 'NGG_PLUGIN_NAME', 'NextCellent Gallery' );

//The main class
if ( !class_exists( 'NextCellentMediaLibraryAddon' ) ) {
	class NextCellentMediaLibraryAddon {
		...
		/**
		 * Get NextCellentMediaLibraryAddon instance.
		 *
		 * @return NextCellentMediaLibraryAddon instance
		 */
		...
		public function check_ngg() {
			if ( !defined('NGGFOLDER') ) {
				if ( is_multisite() ) {
					add_action( 'all_admin_notices', array( &$this, 'required_plugin_notice' ) );
				} else {
		...
		public function required_plugin_notice() {
			echo	'<div id="message" class="error"><p>' . 
					'<strong>[NextCellent Gallery Media Library Addon</strong>] - ' . 
					__( 'this plugin requires <strong>NextCellent Gallery</strong> to be installed and activated first before it can work properly.', 'nextcellent-gallery-media-addon' ) . 
					'</p></div>';
		}
		...
		public function add_ngg_tab( $tabs ) {
			$tabs['frommedialibrary'] = __( 'Media Library', 'nextcellent-gallery-media-addon' );
		...
		public function tab_frommedialibrary() {
			...
			<form id="nggmla-selected-images-form" action="" method="POST">
				<div id="select-gallery"><label for=""><?php _e( 'Add images to:', 'nextcellent-gallery-media-addon' );?></label>
					<select id="togallery" name="togallery">
						<option value="0"><?php _e( 'Choose gallery', 'nextcellent-gallery-media-addon' ); ?>
						</option>
				...
				<p><a id="nggmla-select-images" class="button-secondary" href="#"><?php _e( 'Select Images', 'nextcellent-gallery-media-addon'); ?></a> <span id="image-error" class="nggmla-error"></span>
				</p>
                ...
                <span id="copying" style="display: none;"><?php echo $copying = __( 'Copying...', 'nextcellent-gallery-media-addon' ); ?>
	                <img src="<?php echo NGGALLERY_URLPATH . 'images/ajax-loader.gif'; ?>"
		...
		public function check_safemode( $foldername ) {
			...
			if ( SAFE_MODE ) {
				...
				if ( $script_uid != $folder_uid ) {
					$message =
						sprintf( __( 'SAFE MODE Restriction in effect! You need to create the folder <strong>%s</strong> manually', 'nextcellent-gallery-media-addon' ), esc_html( $foldername ) );
					$message .=
						'<br />' .
						sprintf( __( 'When safe_mode is on, PHP checks to see if the owner (%s) of the current script matches the owner (%s) of the file to be operated on by a file function or its directory', 'nextcellent-gallery-media-addon' ), $script_uid, $folder_uid );
					$msg->error         = true;
...
add_action( 'plugins_loaded', array( NextCellentMediaLibraryAddon::get_instance(), 'init' ) );